...
def _meses_con_adeudo(detalle_plantel: pd.DataFrame) -> str:
    """Devuelve únicamente los meses que conservan saldo pendiente.

    El desglose por concepto se consulta en la hoja Detalle de Cobranza;
    se evita repetirlo en Adeudos para mantener esa vista breve y legible.
    """
    ...
    return ", ".join(
        _etiqueta_mes(mes)
        for mes in sorted(pendientes["MES"].drop_duplicates().tolist())
    )
...
def crear_reporte_adeudos(
    ...
    for clave, grupo in detalle.groupby("CLAVE_PLANTEL", sort=False):
        descripciones.append(
            {
                "CLAVE_PLANTEL": clave,
                "MESES_CON_ADEUDO_REPORTE": _meses_con_adeudo(grupo),
            }
        )
